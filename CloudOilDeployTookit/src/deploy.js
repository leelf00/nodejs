 ...
 * ========================================================= */
"use strict";
var gulp = require('gulp');//gulp自动化部署工具
...
var uglify = require('gulp-uglify');//uglify，JS代码优化工具
var ngAnnotate = require('gulp-ng-annotate');//ng-annotate，AngularJS代码优化工具，和uglify混用时，必须设置uglify({mangle: false})
...
var debug = require('gulp-debug');//调试工具
...
var login_web_b_build = function login_web_b_build() {
		return new Promise((reslove,reject)=>{
			...
			gulp.src(login_web_b_source_path+'/**/*.gif')
				...
				.pipe(gzip())
				.pipe(gulp.dest(login_web_b_deploy_path));
			gulp.src(login_web_b_source_path+'/**/*.webp')
				.pipe(debug())
				//.pipe(imagemin())
				.pipe(gzip())
				.pipe(gulp.dest(login_web_b_deploy_path));				
